...
public class PlayerInputHandler : MonoBehaviour
{
    ...
    public bool UIfirebutton(){
        if(firebutton.Horizontal!=0){
        fireButtonbool = true;
    ...
    public Vector3 GetMoveInput()
    {
        if (CanProcessInput())
        {
            //Vector3 move = new Vector3(Input.GetAxisRaw(GameConstants.k_AxisNameHorizontal), 0f, Input.GetAxisRaw(GameConstants.k_AxisNameVertical));//commented by rahul
    ...
    {
        return GetMouseOrStickLookAxisHorizontal(GameConstants.k_MouseAxisNameHorizontal, GameConstants.k_AxisNameJoystickLookHorizontal);
        //return Vector3(cameramovement.Horizontal,0,cameramovement.Vertical);
        }

    public float GetLookInputsVertical()
    {
        return GetMouseOrStickLookAxisVertical(GameConstants.k_MouseAxisNameVertical, GameConstants.k_AxisNameJoystickLookVertical);
    }
    ...
    public bool GetFireInputHeld()
    {
        ...
        if (CanProcessInput())
        {
            ...
            {
                //return Input.GetButton(GameConstants.k_ButtonNameFire);//Rahul
    ...
    float GetMouseOrStickLookAxisHorizontal(string mouseInputName, string stickInputName)
    {
        if (CanProcessInput())
        {
            // Check if this look input is coming from the mouse
            bool isGamepad = Input.GetAxis(stickInputName) != 0f;
            //float i = isGamepad ? Input.GetAxis(stickInputName) : Input.GetAxisRaw(mouseInputName);//commented by rahul
            float i = isGamepad ? Input.GetAxis(stickInputName) : cameramovement.Horizontal;
    ...
    float GetMouseOrStickLookAxisVertical(string mouseInputName, string stickInputName)
    {
        if (CanProcessInput())
        {
            // Check if this look input is coming from the mouse
            bool isGamepad = Input.GetAxis(stickInputName) != 0f;
            //float i = isGamepad ? Input.GetAxis(stickInputName) : Input.GetAxisRaw(mouseInputName);//commented by rahul
            float i = isGamepad ? Input.GetAxis(stickInputName) : -cameramovement.Vertical;
